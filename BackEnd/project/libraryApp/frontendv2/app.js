...
const loadDashCustomers = async (search = "") => {
  ...
  try {
    ...
    const res = await apiGet("/customer?" + params.toString(), true);
    const list = asList(res.data);
    wrap.innerHTML = list.length
      ? list
          .map(
...
const loadStock = async (search = "") => {
  ...
  wrap.innerHTML = '<p class="msg">جارٍ التحميل...</p>';
  try {
    const res = await apiGet("/stock/books?limit=500", true);
    const list = asList(res.data);
    wrap.innerHTML = list.length
      ? list
          .map(
...
const loadStockMovements = async (bookId) => {
  ...
  $("stock-adjust-form").onsubmit = async (e) => {
    ...
    try {
      const res = await apiPost(`/stock/books/${bookId}`, body, true);
  ...
  try {
    const res = await apiGet(`/stock/books/${bookId}`, true);
